...
SYMPLEGMA_CLUSTER="$(cd "${INVENTORY_DIR}" && terragrunt run-all output cluster_name 2>/dev/null)"
SYMPLEGMA_AWS_REGION="$(cd "${INVENTORY_DIR}" && terragrunt run-all output aws_region 2>/dev/null)"

export SYMPLEGMA_CLUSTER SYMPLEGMA_AWS_REGION

uv run --locked ansible-playbook -i "${INVENTORY_DIR}"/aws_ec2.yml symplegma-init.yml -b -v \
